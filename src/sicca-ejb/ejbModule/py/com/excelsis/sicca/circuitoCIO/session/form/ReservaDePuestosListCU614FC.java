...
import py.com.excelsis.sicca.dto.ConcursoCategoriaAgrDTO;
...
@Name("reservaDePuestosListCU614FC")
public class ReservaDePuestosListCU614FC {
	@In
	...
	private List<PromocionSalarial> listaPromocionSalarial;
	private List<PlantaCargoDetDTO> listaPlantaCargoDto;
	private List<ConcursoPuestoDet> listaPuestosReservados;
	private List<PromocionConcursoAgr> listaCategoriasReservadas;
	private List<ConcursoPuestoAgrDTO> listaPuestosAgrupados;
	private List<ConcursoCategoriaAgrDTO> listaCategoriasDTO;
	private List<ConcursoCategoriaAgrDTO> listaCategoriasAgrupadas;
	...
	DatosEspecificos datosEspecificosMerito = new DatosEspecificos();

	/**
	 * Método que inicializa los datos
	 */
	public void init() {
		datosEspecificosMerito = seleccionUtilFormController
...
//		}
		...
		updateCiudad();
		// updateBarrio();
		llenarListadoPromocionSalarial();
		cargarListadoAgrupadosPromocionSalarial();
		if (idAnx != null && idPromocionSalarial != null)
			actualizarDatosPromocionSalarial();
	}
	...
	public void cargarListadoAgrupados() {
		...
		if (concurso.getDatosEspecificosTipoConc().getIdDatosEspecificos()
				.equals(datosEspecificosMerito.getIdDatosEspecificos()))
			sql += " and cargo.contratado is true ";
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void cargarListadoAgrupadosPromocionSalarial() {
		String sql = "select distinct(puesto_agr.*) "
				+ "from seleccion.concurso_puesto_agr puesto_agr  "
				+ " join seleccion.promocion_concurso_agr promocion_concurso_agr "
				+ "on promocion_concurso_agr.id_concurso_puesto_agr = puesto_agr.id_concurso_puesto_agr "
				+ " join seleccion.promocion_salarial promocion "
				+ "on promocion.id_promocion_salarial = promocion_concurso_agr.id_promocion_salarial "
				+ "join seleccion.concurso concurso "
...
//			sql += " and cargo.contratado is true ";
		...
		listaCategoriasAgrupadas = new ArrayList<ConcursoCategoriaAgrDTO>();
		List<ConcursoPuestoAgr> listaAgr = new ArrayList<ConcursoPuestoAgr>();
		listaAgr = em.createNativeQuery(sql, ConcursoPuestoAgr.class)
				.getResultList();
		for (ConcursoPuestoAgr agr : listaAgr) {

			ConcursoCategoriaAgrDTO dto = new ConcursoCategoriaAgrDTO();
			...
			if (agr.getConcursoPuestoDets() != null
					&& agr.getConcursoPuestoDets().size() > 0) {
				dto.setCantidad(agr.getConcursoPuestoDets().size());
				dto.setPromocionSalarial(agr.getPromocionConcursoAgrs().get(0)
						.getPromocionSalarial());
			} else
				dto.setCantidad(0);
			listaCategoriasAgrupadas.add(dto);
		}
	...
	public void search() throws Exception {
		...
		if (concurso.getDatosEspecificosTipoConc().getDescripcion()
			...
			where1 += " and det.permanente is true ";
			where2 += " and det.permanente is true ";
		}
	...
	private String Sentencia1() {
		...
		/*
		 * if (concurso.getDatosEspecificosTipoConc().getDescripcion()
		 * .equalsIgnoreCase(CONCURSO_SIMPLIFICADO)) sql +=
		 * " join planificacion.cpt cpt on cpt.id_cpt=det.id_cpt ";
		 */
		return sql;
	}
	...
	private void llenarListadoPromocionSalarial() {
		...
		// SE DEBERAN AJUSTAR LOS QUERIES PARA QUE RECUPEREN LAS CATEGORIAS CON ESTADO "CONCURSO" SELECCIONADOS DESDE EL LISTADO DE CATEGORIAS. 
...
//		}
	...
	@SuppressWarnings("unchecked")
	private void cargarListaPromocionSalarial(String sql1, String sql2) {
		listaPromocionSalarial = new ArrayList<PromocionSalarial>();
		listaCategoriasReservadas = new ArrayList<PromocionConcursoAgr>();
		listaCategoriasDTO = new ArrayList<ConcursoCategoriaAgrDTO>();
		listaPromocionSalarial = em.createNativeQuery(sql1, PromocionSalarial.class)
				.getResultList();
		listaCategoriasReservadas = em.createNativeQuery(sql2,
				PromocionConcursoAgr.class).getResultList();
		if (listaCategoriasReservadas.size() > 0) {
			cantReservados = listaCategoriasReservadas.size();
			for (PromocionConcursoAgr r : listaCategoriasReservadas) {
				ConcursoCategoriaAgrDTO dto = new ConcursoCategoriaAgrDTO();
				Long id = r.getPromocionSalarial().getIdPromocionSalarial();
				dto.setPromocionSalarial(r.getPromocionSalarial());
				dto.setReservar(true);
		...
		if (listaPromocionSalarial.size() > 0) {
			for (PromocionSalarial p : listaPromocionSalarial) {
				Boolean esta = false;
				for (PromocionConcursoAgr r : listaCategoriasReservadas) {
					if (r.getPromocionSalarial().equals(p))
						esta = true;
				}
				if (!esta) {

					ConcursoCategoriaAgrDTO dto = new ConcursoCategoriaAgrDTO();
					dto.setPromocionSalarial(p);
					dto.setReservar(false);
					listaCategoriasDTO.add(dto);
	...
	public String getUrlToPageSearch() {
		...
		return "/planificacion/configuracionUoDet/ListarConfiguracionUoDet.xhtml?from=circuitoCIO/reservaDePuestos/ConcursoPuestoDetList&sinEntidadIdSinEntidad="
	...
	@SuppressWarnings("unchecked")
	public void actualizarListaPromocionSalarial(Integer row) {
		ConcursoCategoriaAgrDTO dto = new ConcursoCategoriaAgrDTO();
		...
		listaCategoriasDTO.set(row, dto);
	}
	...
	public String savePromocionSalarial() {
		try {
			...
			for (ConcursoCategoriaAgrDTO dto : listaCategoriasDTO) {

				if (dto.getReservar()) {
					EstadoDet est = buscarEstado("en reserva");
					if (!existeEnPromocionConcursoAgr(dto.getPromocionSalarial(), est)) {
						// buscar

						PromocionConcursoAgr p = new PromocionConcursoAgr();
						p.setActivo(true);
						p.setFechaAlta(new Date());
						p.setUsuAlta(usuarioLogueado.getCodigoUsuario());
						p.setNroOrden(2);
						p.setPromocionSalarial(dto.getPromocionSalarial());
						...
						PromocionSalarial planta = new PromocionSalarial();
						planta = dto.getPromocionSalarial();
						planta.setEstadoCab(est.getEstadoCab());
						em.merge(planta);
				...
				/**/
				EstadoCab estadoCab = obtenerEstadosCabecera("VACANTE");
				if (estaPromocionSalarial(dto.getPromocionSalarial())) {
					PromocionConcursoAgr p = new PromocionConcursoAgr();
					p = recuperarConcursoPromocionSalarial(dto.getPromocionSalarial());
	...
	@SuppressWarnings("unchecked")
	private Boolean existeEnPromocionConcursoAgr(PromocionSalarial planta, EstadoDet est) {
		String sql = "select det.* from "
				+ "seleccion.promocion_concurso_agr det "
				+ "where det.id_promocion_salarial = "
				+ planta.getIdPromocionSalarial() + "and det.id_estado_det = "
				+ est.getIdEstadoDet();
	...
	private Boolean estaPromocionSalarial(PromocionSalarial det) {
		for (PromocionConcursoAgr d : listaCategoriasReservadas) {
			if (d.getPromocionSalarial().getIdPromocionSalarial().toString()
					.equals(det.getIdPromocionSalarial().toString()))
				return true;
		}
		return false;
	}
	...
	public void actualizarDatos() {
		...
		try {
			...
			if (!concurso.getAdReferendum() && anx.getCantDisponible() > 0) {
				...
				/*
				 * msg = msg +
				 * ", la categoria seleccionada no se encuentra disponible"; if
				 * (!mensaje.trim().isEmpty()) msg += ", " + mensaje;
				 */
	...
	public void actualizarDatosPromocionSalarial() {
		SinAnx anx = new SinAnx();
		anx = em.find(SinAnx.class, idAnx);

		Cpt cpt = new Cpt();
	
		PromocionSalarial promocionSalarial = new PromocionSalarial();
		...
		Long idC = null;
		if (promocionSalarial.getCpt() != null)
			idC = promocionSalarial.getCpt().getIdCpt();
		...
		buscarMontoPromocionSalarial(anx);
		
		try {
			

			promocionSalarial.setObjCodigo(new Long(111));
			promocionSalarial.setCategoria(anx.getCtgCodigo());
			Date fecha = new Date();
			Integer anho = fecha.getYear() + 1900;
			promocionSalarial.setAnho(new Long (anho));
			if (monto != null)
				promocionSalarial.setMonto(new Long (monto.intValue()));
			promocionSalarial.setFechaAlta(fecha);
			promocionSalarial.setUsuAlta(usuarioLogueado.getCodigoUsuario());
			Referencias refReservado = referenciasUtilFormController
					.getReferencia("ESTADOS_CATEGORIA", "RESERVADO");
			Referencias refPendiente = referenciasUtilFormController
					.getReferencia("ESTADOS_CATEGORIA", "PENDIENTE");
			if ( anx.getCantDisponible() > 0) {
				promocionSalarial.setDisponible(true);
				...
				promocionSalarial.setActivo(true);
//				if (refPendiente != null)
//					promocionSalarial.setEstado(refPendiente.getValorNum());
				em.merge(promocionSalarial);
				em.flush();
				statusMessages.clear();
				String msg = SeamResourceBundle.getBundle().getString(
	...
	@SuppressWarnings({ "deprecation", "unchecked" })
	private void buscarMontoPromocionSalarial(SinAnx sinAnx) {
		Integer cat_grupo = sinAnx.getCatGrupo();
	...
	private PromocionConcursoAgr recuperarConcursoPromocionSalarial(PromocionSalarial det) {
		for (PromocionConcursoAgr d : listaCategoriasReservadas) {
			if (d.getPromocionSalarial().equals(det))
				return d;
		}
		return null;
	}
	...
	/*
	 * private Boolean esConcursoSimplificado(Long idTipo) { DatosEspecificos
	 * tipoCon = em.find(DatosEspecificos.class, idTipo); if
	 * (tipoCon.getDescripcion().toLowerCase() .equals("concurso simplificado"))
	 * return true;
	 * 
	 * return false; }
	 */
	...
	public List<ConcursoCategoriaAgrDTO> getListaCategoriasDTO() {
	...
	public void setListaCategoriasDTO(
			List<ConcursoCategoriaAgrDTO> listaCategoriasDTO) {
	...
	public List<ConcursoCategoriaAgrDTO> getListaCategoriasAgrupadas() {
	...
	public void setListaCategoriasAgrupadas(
			List<ConcursoCategoriaAgrDTO> listaCategoriasAgrupadas) {
		this.listaCategoriasAgrupadas = listaCategoriasAgrupadas;
	}
