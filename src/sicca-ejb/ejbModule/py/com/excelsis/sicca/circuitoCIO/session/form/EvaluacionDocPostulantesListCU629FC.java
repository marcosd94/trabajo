...
@Name("evaluacionDocPostulantesListCU629FC")
public class EvaluacionDocPostulantesListCU629FC {
	...
	private void validarOee(Concurso concurso) {
		...
		if (!seguridadUtilFormController.validaEstado(concursoPuestoAgr.getIdConcursoPuestoAgr(), "A EVALUAR DOCUMENTOS")) {
			throw new AuthorizationException(SeamResourceBundle.getBundle().getString("GENERICO_OEE_NO_VALIDA"));
		}
	}
	...
	public String init() {
		...
		obs = new Observacion();
		estadoGrupoDesierto = obtenerEstadoGrupo("ESTADOS_GRUPO", "GRUPO DESIERTO");
...
//				validarEstado();
		...
		if (cantidadPostulantes(concursoPuestoAgr.getIdConcursoPuestoAgr()) <= 1) {
			...
			reponerCategorias();
			bloquearTodo = true;
			statusMessages.add(Severity.ERROR, "Este grupo se declara desierto por no contar con Postulantes suficientes.");
			return "desierto";
		}
		bloquearTodo = false;
		...
		from = "circuitoCIO/evaluacionDocumentalPostulante/EvalDocumentalCabList";
		if (mensaje != null && !mensaje.trim().isEmpty()) {
			if (aprobo.equals("SI"))
				mensaje += " " + SeamResourceBundle.getBundle().getString("CU41_msg_aprobado");
			else
				mensaje += " " + SeamResourceBundle.getBundle().getString("CU41_msg_noaprobado");
	...
	public void reponerCategorias() {
		reponerCategoriasController.reponerCategorias(concursoPuestoAgr, "GRUPO", "CONCURSO");
	}
...
public void imprimirCopiaLegajo(EvalDocumentalCab evalDocumentalCab) throws Exception {
		...
		String sql = " select * from general.documentos where nombre_fisico_doc like 'Copia_legajo_al_postularse_%' "
				+ " and id_persona =  "  + evalDocumentalCab.getPostulacion().getPersonaPostulante().getPersona().getIdPersona() 
				+ " and id_concurso =  " + evalDocumentalCab.getPostulacion().getConcursoPuestoAgr().getConcurso().getIdConcurso();
	...
	public Integer cantidadPostulantes(Long idGrupo) {
		String sql =
			"select post.* from seleccion.postulacion post " + "where post.activo is true "
				+ "and post.usu_cancelacion is null " + "and post.id_concurso_puesto_agr = "
				+ idGrupo;
	...
	public void declararDesierto() {
		...
		cambiarEstadoPuestos(traerGrupoPuestoDet(concursoPuestoAgr.getIdConcursoPuestoAgr()));
		if (!Utiles.vacio(observacion)) {
			...
			obs.setIdTaskInstance(jbpmUtilFormController.getIdTaskInstanceActual(concursoPuestoAgr.getIdProcessInstance()));
			em.persist(obs);
		}
		// Se pasa a la siguiente tarea
		try {
			jbpmUtilFormController.setConcursoPuestoAgr(concursoPuestoAgr);
			jbpmUtilFormController.setActividadActual(ActividadEnum.CIO_EVALUACION_DOCUMENTAL);
		...
		if (admFecRecPosFC == null) {
			admFecRecPosFC =
				(py.com.excelsis.sicca.seleccion.session.form.AdmFecRecPosFC) Component.getInstance(AdmFecRecPosFC.class, true);
		}
		admFecRecPosFC.enviarMail2(concursoPuestoAgr);
		admFecRecPosFC.insertarPublicacionPortal(concursoPuestoAgr.getIdConcursoPuestoAgr(), concursoPuestoAgr.getConcurso().getIdConcurso(), admFecRecPosFC.genTextoPublicacion2());
	...
	public void logicaDisminuirPuestos(Integer cantPostulantes, String elFrom) {
		...
		if (cantVacancias.intValue() >= cantPostulantes.intValue()) {
			Integer cantDisminuir = (cantVacancias.intValue() - cantPostulantes.intValue()) + 1;
			mensajeModal =
				"Debe sacar "
					+ cantDisminuir
					+ " puesto/s del grupo por no contar con la cantidad suficiente de Postulantes.<br/>"
					+ "øDesea sacar los puestos ahora? ";
			viewDisminuirPuestos(concursoPuestoAgr.getIdConcursoPuestoAgr(), cantVacancias, cantPostulantes, elFrom);
			mostrarModal = true;
			statusMessages.add(Severity.ERROR, "Debe sacar " + cantDisminuir
	...
	public Integer cantVacancias() {
		Query q =
			em.createQuery("select count(ConcursoPuestoDet) from ConcursoPuestoDet ConcursoPuestoDet "
	...
	public String viewDisminuirPuestos(Long grupo, Integer cantVacancias, Integer cantPostulantes, String from) {
		view451 =
			"/seleccion/admDisPue/admDisPue451.seam?idConcursoPuestoAgr=" + grupo
				+ "&cantPostulantes=" + cantPostulantes + "&cantVacancias=" + cantVacancias
				+ "&from=" + from + "&codActividad=" + codActividad;
		return view451;
	}

	private void todosFueronEvaluados() {
		String cad =
			"select eval_cab.* " + "from seleccion.eval_documental_cab eval_cab "
				+ "join seleccion.concurso_puesto_agr agr  "
				+ "on agr.id_concurso_puesto_agr = eval_cab.id_concurso_puesto_agr  "
				+ "where eval_cab.activo is true " + "and eval_cab.evaluado is true "
				+ "and eval_cab.tipo = 'EVALUACION DOCUMENTAL' "
				+ "and agr.id_concurso_puesto_agr = " + concursoPuestoAgr.getIdConcursoPuestoAgr();

		List<EvalDocumentalCab> lista = new ArrayList<EvalDocumentalCab>();
		lista = em.createNativeQuery(cad, EvalDocumentalCab.class).getResultList();
		if (lista.size() > 0) {
	...
	public String getQuery() {
		String query =
			"select evalDocumentalCab from EvalDocumentalCab evalDocumentalCab "
				...
				+ "and agr.idConcursoPuestoAgr = " + concursoPuestoAgr.getIdConcursoPuestoAgr();
		if (fechaDesde != null && fechaHasta != null)
			query +=
				"and date(evalDocumentalCab.fechaEvaluacion) >= #{evalDocumentalCabList.fechaDesde} "
					+ "date(evalDocumentalCab.fechaEvaluacion) <= #{evalDocumentalCabList.fechaHasta}";
		if (observacion != null && !observacion.trim().isEmpty())
			query +=
				"lower(evalDocumentalCab.observacion) like lower(concat('%', concat(" + observacion
					+ ",'%')))";
	...
	private void buscarCantEvaluados() {
		String cad =
			"select eval_cab.* " + "from seleccion.eval_documental_cab eval_cab  "
				...
				+ "where eval_cab.activo is true  " + "and eval_cab.evaluado is true  "
				+ "and post.activo is true  " + "and eval_cab.tipo = 'EVALUACION DOCUMENTAL' "
				+ "and post.usu_cancelacion is null " + "and eval_cab.id_concurso_puesto_agr = "
				+ concursoPuestoAgr.getIdConcursoPuestoAgr();
		List<EvalDocumentalCab> lista = new ArrayList<EvalDocumentalCab>();
		lista = em.createNativeQuery(cad, EvalDocumentalCab.class).getResultList();
		if (lista.size() > 0)
	...
	public void searchAll() {
		evalDocumentalCabList.setIdConcursoPuestoAgr(concursoPuestoAgrHome.getInstance().getIdConcursoPuestoAgr());
		evalDocumentalCabListado = evalDocumentalCabList.limpiarResultados();
		fechaDesde = null;
		fechaHasta = null;
		observacion = null;
	}
	...
	private void buscarMatrizDocConfigEnc() {
		String sql =
			"select matriz_enc.* " + "from seleccion.matriz_doc_config_enc matriz_enc "
				+ "join seleccion.concurso_puesto_agr agr "
				+ "on agr.id_concurso_puesto_agr = matriz_enc.id_concurso_puesto_agr "
				+ "where matriz_enc.activo is true " + "and agr.id_concurso_puesto_agr = "
				+ concursoPuestoAgr.getIdConcursoPuestoAgr();

		listaMatrizDocCOnfigEnc = new ArrayList<MatrizDocConfigEnc>();
		listaMatrizDocCOnfigEnc =
			em.createNativeQuery(sql, MatrizDocConfigEnc.class).getResultList();
	}
	...
	private void buscarMatrizDocConfigDet() {
		String sql =
			"select matriz_det.* from " + "seleccion.matriz_doc_config_det matriz_det "
				...
				+ "join seleccion.concurso_puesto_agr agr "
				+ "on agr.id_concurso_puesto_agr = matriz_enc.id_concurso_puesto_agr "
				+ "where matriz_enc.activo is true " + "and matriz_det.evaluacion_doc is true "
				+ "and agr.id_concurso_puesto_agr = " + concursoPuestoAgr.getIdConcursoPuestoAgr()
				+ " order by matriz_det.nro_orden";

		listaMatrizDocCOnfigDet = new ArrayList<MatrizDocConfigDet>();
		listaMatrizDocCOnfigDet =
			em.createNativeQuery(sql, MatrizDocConfigDet.class).getResultList();
	}
	...
	private void buscarPostulacion() {
		String sql =
			"select post.* " + "from seleccion.postulacion post "
				+ "join seleccion.concurso_puesto_agr agr "
				+ "on agr.id_concurso_puesto_agr = post.id_concurso_puesto_agr "
				+ "where post.activo is true " + "and post.usu_cancelacion is null "
				+ "and agr.id_concurso_puesto_agr = " + concursoPuestoAgr.getIdConcursoPuestoAgr();
		listaPostulacion = new ArrayList<Postulacion>();
		listaPostulacion = em.createNativeQuery(sql, Postulacion.class).getResultList();
	}
	...
	private Boolean estaPostulacion(Long id) {
		String sql =
			"select cab.* from seleccion.eval_documental_cab cab "
				...
				+ "where post.activo is true " + "and post.usu_cancelacion is null "
				+ "and cab.tipo = 'EVALUACION DOCUMENTAL' " + "and cab.id_concurso_puesto_agr = "
				+ concursoPuestoAgr.getIdConcursoPuestoAgr() + " and post.id_postulacion = " + id;
		List<EvalDocumentalCab> lista = new ArrayList<EvalDocumentalCab>();
	...
	private String guardarDatos() {
		...
		for (Postulacion p : listaPostulacion) {
			if (!estaPostulacion(p.getIdPostulacion())) {
				...
				listaEvalDocumentalDet = buscarDocumentalDet(evalDocumentalCabHome.getInstance());
				actualizarDatos();
	...
	/**
	 * Actualiza la tabla eval_documental_det teniendo en cuenta el tipo de documento
	 */
	private void actualizarDatos() {
		for (EvalDocumentalDet d : listaEvalDocumentalDet) {
			...
			for (PostulacionAdjuntos adj : listaDocumentosAdjPostulante) {
				...
				if (datosEspAdj.getIdDatosEspecificos().equals(datosEspEval.getIdDatosEspecificos())) {
					esta = true;
	 ...
	 */
	@SuppressWarnings("unchecked")
	private List<PostulacionAdjuntos> buscarDocumentosAdjPostulante(Postulacion postulacion) {
		try {
			String sql =
				"select adj.* " + "from seleccion.postulacion_adjuntos adj "
					+ "join seleccion.postulacion postulacion "
					+ "on postulacion.id_postulacion = adj.id_postulacion "
					+ "where adj.activo is true " + "and postulacion.id_postulacion = "
					+ postulacion.getIdPostulacion();

			return em.createNativeQuery(sql, PostulacionAdjuntos.class).getResultList();
		} catch (Exception e) {
			// TODO: handle exception
		}
		return null;
	...
	@SuppressWarnings("unchecked")
	private List<EvalDocumentalDet> buscarDocumentalDet(EvalDocumentalCab evalDocumentalCab) {
		try {
			String sql =
				"select documental_det.* "
			...
			return em.createNativeQuery(sql, EvalDocumentalDet.class).getResultList();
		} catch (Exception e) {
			// TODO: handle exception
		}
		return null;
	...
	private Integer countAprobados() {
		Query q =
			em.createQuery("select count(EvalDocumentalCab) from EvalDocumentalCab EvalDocumentalCab "
	...
	public EstadoDet obtenerEstadosVarios(String detDesc, String cabDesc) {
		Query q =
			em.createQuery("Select EstadoDet from EstadoDet EstadoDet "
				+ "where EstadoDet.activo is true and EstadoDet.descripcion = '" + detDesc + "' "
	...
	public EstadoCab obtenerEstadosCabecera(String cabDesc) {
		Query q =
			em.createQuery("Select EstadoCab from EstadoCab EstadoCab "
				+ "where EstadoCab.activo is true and EstadoCab.descripcion = '" + cabDesc + "' ");
	...
	public void cambiarEstadoPuestos(List<ConcursoPuestoDet> lista) {
		...
		for (ConcursoPuestoDet concursoPuesto : lista) {
			...
			estado.setIdEstadoDet(obtenerEstadosVarios("DESIERTO", "CONCURSO").getIdEstadoDet());
	...
	public Boolean logica2(Integer canAprobados, Integer cantVacancias, String elFrom) {

		if (canAprobados.intValue() >= 0 && canAprobados.intValue() < 2) {
			/* Declarar DESIERTO el Grupo: */
			/* Actualizar el estado del Grupo: */
			cambiarEstadoGrupo(concursoPuestoAgr, "ESTADOS_GRUPO", "GRUPO DESIERTO", false);
			/* Actualizar el ESTADO de los PUESTOS: */
			cambiarEstadoPuestos(traerGrupoPuestoDet(concursoPuestoAgr.getIdConcursoPuestoAgr()));
			/* Enviar mails a los integrantes de la ComisiÛn de SelecciÛn */
			enviarMailComision(concursoPuestoAgr);
	...
	public Boolean logica(Integer canAprobados, Integer cantVacancias, String elFrom) {

		if (canAprobados.intValue() > 1 && canAprobados.intValue() <= cantVacancias.intValue()) {
			logicaDisminuirPuestos(canAprobados, elFrom);
			continuaValSgteTarea = false;
		} else {
			continuaValSgteTarea = true;
	...
	public void enviarMailComision(ConcursoPuestoAgr concursoPuestoAgr) {
		...
		if (admFecRecPosFC == null) {
			admFecRecPosFC = (AdmFecRecPosFC) Component.getInstance(AdmFecRecPosFC.class, true);
		}
		List<ComisionSeleccionDet> lDestinatarios =
			admFecRecPosFC.traerDestinatarios(concursoPuestoAgr.getIdConcursoPuestoAgr());
		String dirEmail = null;
		for (ComisionSeleccionDet o : lDestinatarios) {
			dirEmail = o.getPersona().getEMail();
			String asunto =
				"Declarar Desierto el Concurso o hacer Prorroga por ExcepciÛn ñ con AprobaciÛn de la SFP";
			String texto = "";
			try {
				texto =
					texto
	...
	private Integer cantEvaluacionesCerradas() {
		String sql =
			"select distinct(cab.*) " + "from seleccion.eval_documental_cab cab "
				+ "join seleccion.postulacion post "
				+ "on post.id_postulacion = cab.id_postulacion "
				+ "where (cab.evaluado is null or cab.evaluado is false) "
				+ "and cab.tipo = 'EVALUACION DOCUMENTAL' " + "and cab.id_concurso_puesto_agr = "
				+ concursoPuestoAgr.getIdConcursoPuestoAgr()+" and post.activo is true";
	...
	public String nextTask() {
		...
		if (cantEvaluacionesCerradas() > 0) {
			statusMessages.add(Severity.ERROR, "Debe cerrar las evaluaciones de todos los Postulantes.");
			return null;
		}
		try {
			Integer cantVacancias = cantVacancias();
			...
			if (cantidadAprobados == null || cantidadAprobados <= 1) {
				...
				bloquearTodo = true;
				statusMessages.clear();
				statusMessages.add(Severity.ERROR, "Este grupo se declara desierto por no contar con la cantidad suficiente de Postulantes aprobados.");
				return "desierto";
			}
			if (cantVacancias == null
				|| (cantidadAprobados != null && cantidadAprobados > cantVacancias.intValue())) {
				mostrarModal = false;
				continuaValSgteTarea = true;

			} else {
				mostrarModal = true;
				logica(countAprobados(), cantVacancias(), "circuitoCIO/evaluacionDocumentalPostulante/EvalDocumentalCabList");
			...
			if (continuaValSgteTarea) {
				...
				ref =
					referenciasUtilFormController.getReferencia("ESTADOS_GRUPO", "EVALUACION DOCUMENTAL");
				if (ref != null) {
					...
					if (cargoDet != null) {
						...
						estadoDet =
							seleccionUtilFormController.buscarEstadoDet("concurso", "evaluacion documental");
						if (estadoDet != null) {
							cargoDet.setEstadoDet(estadoDet);
							em.merge(cargoDet);
							List<ConcursoPuestoDet> listaPuestosDetActualizar = new ArrayList<ConcursoPuestoDet>();
							listaPuestosDetActualizar =	traerGrupoPuestoDet(concursoPuestoAgr.getIdConcursoPuestoAgr());
		...
		if (continuaValSgteTarea) {
			...
			try {
				for (EvalDocumentalCab envio : listaEnvio) {
					enviarEmails(envio.getPostulacion().getPersonaPostulante(), envio.getAprobado(), envio.getObservacion());
				}
			} catch (Exception e) {
				// TODO: handle exception
			}
			...
			jbpmUtilFormController.setConcursoPuestoAgr(concursoPuestoAgr);
			jbpmUtilFormController.setActividadActual(ActividadEnum.CIO_EVALUACION_DOCUMENTAL);
			jbpmUtilFormController.setActividadSiguiente(ActividadEnum.CIO_ELABORAR_PUBLICACION_LISTA_LARGA);
	...
	private PlantaCargoDet buscarPuesto() {
		String sql =
			"select distinct(cargo_det.*) " + "from planificacion.planta_cargo_det cargo_det "
		...
		listaPlanta = em.createNativeQuery(sql, PlantaCargoDet.class).getResultList();
	...
	private List<EvalDocumentalCab> buscarTodaslasCab() {
		String sql =
			"select doc_cab.* " + "from seleccion.eval_documental_cab doc_cab "
				+ "join seleccion.concurso_puesto_agr agr "
				+ "on agr.id_concurso_puesto_agr = doc_cab.id_concurso_puesto_agr "
				+ "where doc_cab.activo is true " + "and agr.id_concurso_puesto_agr = "
				+ concursoPuestoAgr.getIdConcursoPuestoAgr();
		List<EvalDocumentalCab> listaEval = new ArrayList<EvalDocumentalCab>();
		listaEval = em.createNativeQuery(sql, EvalDocumentalCab.class).getResultList();
		return listaEval;
	}
	...
	public void cambiarEstadoGrupo(ConcursoPuestoAgr grupo, String dominio, String descAbrev, Boolean activo) {
	...
	public void setListaMatrizDocCOnfigEnc(List<MatrizDocConfigEnc> listaMatrizDocCOnfigEnc) {
	...
	public void setListaMatrizDocCOnfigDet(List<MatrizDocConfigDet> listaMatrizDocCOnfigDet) {
	...
	public void setListaDocumentosAdjPostulante(List<PostulacionAdjuntos> listaDocumentosAdjPostulante) {
	...
	public void setListaEvalDocumentalDet(List<EvalDocumentalDet> listaEvalDocumentalDet) {
