...
@Name("publicarConcursoSFPCU625FC")
public class PublicarConcursoSFPCU625FC {
	@In
	...
	private void validarOee() {
		...
		seguridadUtilFormController.verificarPerteneceOee(null, concursoPuestoAgr.getIdConcursoPuestoAgr(), seguridadUtilFormController.estadoActividades("ESTADOS_GRUPO", "A PUBLICAR")
			+ "", ActividadEnum.CIO_APROBAR_PUBLICACION.getValor());
	}
	...
	public void init() {
		...
		direccionFisica =
			separator + "SICCA" + separator + anho + separator + "OEE" + separator
				+ concurso.getConfiguracionUoCab().getIdConfiguracionUo() + separator
				+ concurso.getDatosEspecificosTipoConc().getIdDatosEspecificos() + separator
	...
	public String getUrlToPagePrototipoAlternativo() {
		return "/circuitoCIO/publicarConcursoPortalSFP/SolicitarAprobacionSFP.xhtml?fromToPage=circuitoCIO/publicarConcursoPortalSFP/PublicacionConcursoCabList&idConcursoPuestoAgr="
	...
	public boolean siguienteTarea(ConcursoPuestoAgr ag, String valor) {
		try {
			...
			jbpmUtilFormController.setActividadActual(ActividadEnum.CIO_APROBAR_PUBLICACION);
			...
			if (valor.equals("revision")) {
				jbpmUtilFormController.setActividadSiguiente(ActividadEnum.CIO_REVISION_PUBLICACION_OEE);
	...
	private PublicacionConcurso publicacionCoDelGrupo() {
		try {
			PublicacionConcurso pc =
				(PublicacionConcurso) em.createQuery("Select p from PublicacionConcurso p where "
