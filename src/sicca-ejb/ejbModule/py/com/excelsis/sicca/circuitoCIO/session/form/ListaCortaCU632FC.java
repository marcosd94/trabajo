...
@Name("listaCortaCU632FC")
public class ListaCortaCU632FC {
	@In
	...
	public List<EvalReferencialPostulante> buscarListaElegibles() {
		String sql = "select eval_ref.*  "
				...
				+ "where eval_ref.seleccionado is false  "
				+ "and eval_ref.lista_corta is true "
	...
	private void validarOee() {
		...
		seguridadUtilFormController.verificarPerteneceOeeCIO(
				...
				ActividadEnum.CIO_ELABORAR_PUBLICACION_LISTA_CORTA.getValor());
	}
	...
	private void declararDesierto() {
		...
		try {
			...
			jbpmUtilFormController
					.setActividadActual(ActividadEnum.CIO_ELABORAR_PUBLICACION_LISTA_CORTA);
	...
	public String getUrlToListaElegibles() {
		return "/circuitoCIO/listaCorta/ListaElegibles.seam?concursoPuestoAgrIdConcursoPuestoAgr="
				+ concursoPuestoAgr.getIdConcursoPuestoAgr();
	}
	...
	private String genTextoPublicacion() {
		...
		String spanC = "</span>";
		String br = "</br>";
		texto.append(hr + fechaPublicacion + br);
		if (esTerna()) {
			SimpleDateFormat sdfHora = new SimpleDateFormat("HH:mm");
	...
	try {
		...
		if (listaNoSeleccionados != null && listaNoSeleccionados.size() > 0) {
			...
			for (Persona per : listaNoSeleccionados) {
				...
				this.asuntoNoSeleccion = "Notificación de no selección - Descalificación ";
				...
				String texto = "<p align=\"justify\"> <b> Estimado/a    "
						...
						+ ":</p> "
						+ "</b> <p align=\"justify\"> Lamentamos comunicarle que ha quedado descalificado del proceso de seleccion, ya que Ud. NO ha alcanzado el puntaje mínimo necesario para el concurso: "
						+ " </p> "
						+ "<p> <b>"
						+ concurso.getNumero()
						+ "/"
	...
	private String recuperarTexto(EvalReferencialPostulante referen) {
		...
		if (tipoMT.equals("T") && referen.getListaCorta()) {
			...
			asunto="Notificación de pre-selección";
			text = text
					...
					+ pers.getApellidos()
					+ ":</p> "
					+ "</b> <p align=\"justify\"> Le comunicamos que ha sido preseleccionado/a para formar parte de la(s) terna(s) para el Concurso: "
					+ " </p> "
					+ "<p> <b>"
					+ concurso.getNumero()
					+ "/"
					...
					+ concursoPuestoAgr.getDescripcionGrupo()
					+ "</b> </p> "
					+ "<p align=\"justify\">Le comunicamos que las entrevistas con la M&aacute;xima Autoridad Institucional est&aacute;n fijadas segun el siguiente cronograma:</p> "
					+ "<blockquote><b>Fecha: </b> "+fechaConv+"</blockquote>"
					+ "<blockquote><b>Lugar: </b> "+listaARecuperar.getLugar()+"</blockquote>"
					+ "<blockquote><b>Direcci&oacute;n: </b> "+ listaARecuperar.getDireccion() + " - "+ listaARecuperar.getCiudad().getDescripcion()+".</blockquote>"
					+ "<blockquote><b>A partir de las</b> "+horaInicio+"</p><b> hasta las </b>"+horaFin +" hs."
					+ "<p> Obs.: " + listaARecuperar.getObservacion() + "</p> "
					+ "<p><b> Atentamente, <br/>"
					+ configuracionUoCab.getDenominacionUnidad() + "</b></p> "
					+ "<p> Para mayor informaci&oacute;n comunicarse con: "
					+ informacion() + " o acceda al Portal Paraguay Concursa haciendo <a href=\"https://www.paraguayconcursa.gov.py/\">click aqu&iacute;</a>.</p> ";
		...
		if (tipoMT.equals("T") && referen.getListaCorta() != null && !referen.getListaCorta()) {
			...
			asunto="Notificación de no selección";
			text = text
					+ "<p align=\"justify\"> <b> Estimado/a    "
					+ pers.getNombres()
					+ " "
					+ pers.getApellidos()
					+ "</b> "
					+ "</p> <p align=\"justify\"> Lamentamos comunicarle que no ha sido preseleccionado/a en el Concurso: </p> "
					+ "<p> <b>"
					+ concurso.getNumero()
					+ "/"
					+ concurso.getAnhio()
		...
		if (referen.getSeleccionado() != null && tipoMT.equals("M")
			...
			String _obs="";
			asunto="Notificación de Selección";
			...
			text = text
					...
					+ pers.getApellidos()
					+ ":</b> "
					+ "</p> <p align=\"justify\"> Le comunicamos que Ud. ha obtenido "
					+ "uno de los mejores puntajes y HA SIDO SELECCIONADO/A"
					...
					+ concursoPuestoAgr.getDescripcionGrupo()
					+ "</b> </p> "
					+"<p align=\"center\">&#161;&#161;&#161;FELICITACIONES&#33;&#33;&#33;</p>"
					+ "<p> <b> "+_obs
		...
		if (referen.getSeleccionado() != null && tipoMT.equals("M")
			...
			text = text
					...
					+ pers.getApellidos()
					+ "</p> "
					+ "</b> <p align=\"justify\"> Lamentamos comunicarle que no ha sido preseleccionado/a en el Concurso: </p> "
					+ "<p> <b>"
					+ concurso.getNumero()
					+ "/"
					+ concurso.getAnhio()
					...
					+ "<p align=\"center\"> <b>"
					+ concursoPuestoAgr.getDescripcionGrupo()
					+ "</b> </p> <p>Como Ud. ha alcanzado el puntaje mínimo necesario ha quedado registrado/a en el Banco de Personas Elegibles del Concurso.</p> "
					+ "<p align=\"justify\">Le agradecemos el inter&eacute;s que ha mostrado para este Concurso, y deseamos poder contar con usted para nuestros pr&oacute;ximos concursos.</p> "
	...
	public String nextTask() {
		...
		try {
			...
			if (tipoMT.equals("T")) {
				...
				statusMessages
						.add(Severity.ERROR,
								"Ocurrio un error al intentar ir a la siguiente actividad. Verifique el estado del grupo.");
				return "null";
			}
			// Se pasa a la siguiente tarea
			if (jbpmUtilFormController.nextTask()) {
	...
	public String getUrlToPageConvocatoriaFinal() {
		...
		return "/circuitoCIO/listaCorta/ListaCortaEdit.xhtml?fromToPage=circuitoCIO/listaCorta/ElaboracionListaCorta&concursoPuestoAgrIdConcursoPuestoAgr="
				+ concursoPuestoAgr.getIdConcursoPuestoAgr();
	}

	public String getUrlToPageElaborarListaCorta() {
		return "/circuitoCIO/listaCorta/ElaboracionListaCorta.xhtml?fromToPage=circuitoCIO/listaCorta/ListaCortaEdit&concursoPuestoAgrIdConcursoPuestoAgr="
				+ concursoPuestoAgr.getIdConcursoPuestoAgr();
	}

	public String getUrlToPageLista() {
		return "/circuitoCIO/listaCorta/ElaboracionListaCorta.xhtml?fromToPage=circuitoCIO/listaCorta/ElaboracionListaCorta&concursoPuestoAgrIdConcursoPuestoAgr="
				+ concursoPuestoAgr.getIdConcursoPuestoAgr();
	}
		...
		try {
			for (Object[] obj : lista) {
				...
				if(obj[2] != null && obj[3] != null){
					if(obj[2].toString().equals("SI") && obj[3].toString().equals("SI"))
