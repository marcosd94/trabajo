...
@Name("admCargaGrupoCU616FC")
public class AdmCargaGrupoCU616FC {
	@In(create = true)
	...
	private List<PromocionConcursoAgr> listaPromocionConcursoAgr = new ArrayList<PromocionConcursoAgr>();
	...
	public void initPromocionSalarial() {
		...
		if (idConcursoPuestoAgr != null) {
			...
			nivelEntidad = new SinNivelEntidad();
			concurso = concursoPuestoAgr.getConcurso();
			listaPromocionConcursoAgr = new ArrayList<PromocionConcursoAgr>();
			buscarListaPromocionSalarial();
			//muestraBancoPerfiles();
			if (concurso != null) {
				/* Incidencia 1014 */
				validarOee();
...
//						"No se detectaron cambios que guardar.");
	...
	private void validarOee() {
		...
		seguridadUtilFormController.verificarPerteneceOeeCIO(
				...
				seguridadUtilFormController.estadoActividades("ESTADOS_GRUPO",
						"INICIADO CIRCUITO") + "", ActividadEnum.CIO_CARGA_GRUPO
						.getValor());
	...
	@SuppressWarnings("unchecked")
	private void buscarListaPromocionSalarial() {
		String sql =
			"select det.* " + "from seleccion.promocion_concurso_agr det "
				+ "where det.activo is true " + "and det.id_concurso_puesto_agr = "
				+ concursoPuestoAgr.getIdConcursoPuestoAgr();
		listaPromocionConcursoAgr = em.createNativeQuery(sql, PromocionConcursoAgr.class).getResultList();
	}
	...
	public void search() {
		...
		String where = " where agr.id_concurso_puesto_agr = " + idConcursoPuestoAgr+ " and cargo_det.permanente is true ";
	...
	public String nextTask() {
		...
		if (!concurso.getPromocion() && !validate(concursoPuestoAgr)) {
			return null;
		}
		...
		jbpmUtilFormController.setActividadActual(ActividadEnum.CIO_CARGA_GRUPO);
		/************************* <MODIFICACION> *********************************/
		/************************* MODIFICADO: 21/04/2014 *************************/
		/************************* AUTOR: ENRIQUE CESPEDES ***********************/
		/* Se Agregó la modificacion para replicar lo que realizado en el proceso de CPO
		 * 
		 * 
		 *if (concursoPuestoAgr.getHomologar() == null || concursoPuestoAgr.getHomologar()) { // Se asume que por defecto se envie a homologacion si no hay nada cargado
			jbpmUtilFormController.setActividadSiguiente(ActividadEnum.CIO_ENVIAR_HOMOLOGACION);
		...
		// Guardar HOMOLOGACION_PERFIL_MATRIZ AGREGADO EC; se copio el codigo del proceso de CPO (AdmCargaGrupoFormController)
				Date fecha = new Date();
	...
	public String nextTaskPromocionSalarial() {
		...
		// Guardar HOMOLOGACION_PERFIL_MATRIZ AGREGADO EC; se copio el codigo del proceso de CPO (AdmCargaGrupoFormController)
				...
				PromocionConcursoAgr promocionConcursoAgr = concursoPuestoAgr.getPromocionConcursoAgrs().get(0);
				
				homologacionPerfilMatriz.setCpt(promocionConcursoAgr.getPromocionSalarial().getCpt());
				em.persist(homologacionPerfilMatriz);
	...
	public List<PromocionConcursoAgr> getListaPromocionConcursoAgr() {
		return listaPromocionConcursoAgr;
	}


	public void setListaPromocionConcursoAgr(
			List<PromocionConcursoAgr> listaPromocionConcursoAgr) {
		this.listaPromocionConcursoAgr = listaPromocionConcursoAgr;
	}
