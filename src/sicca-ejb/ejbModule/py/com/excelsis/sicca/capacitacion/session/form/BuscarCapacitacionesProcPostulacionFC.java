...
import py.com.excelsis.sicca.capacitacion.session.CapacitacionPocPostList;
...
@Name("buscarCapacitacionesProcPostulacionFC")
public class BuscarCapacitacionesProcPostulacionFC implements Serializable {
	...
	@In(create = true)
	CapacitacionPocPostList capacitacionPocPostList;
	...
	private List<CapacitacionPocPost> capacPocPostListas= new ArrayList<CapacitacionPocPost>();
	...
	public void search() {
		capacitacionPocPostList.setIdCiudad(idCiudad);
		capacitacionPocPostList.setIdDepartamento(codDepartamento);
		capacitacionPocPostList.setIdPais(codPais);
		capacitacionPocPostList.setIdTipoCapa(idTipoCapacitacion);
		capacitacionPocPostList.getCapacitacionPocPost().setModVal(modalidadCapaSeleccione.getValor());
		if(sugestion!=null && !sugestion.trim().equals(""))
			capacitacionPocPostList.getCapacitacionPocPost().setNombre(sugestion);
		else
			capacitacionPocPostList.getCapacitacionPocPost().setNombre(null);
		capacPocPostListas=capacitacionPocPostList.listaResultados();
	...
	public void searchAll() {
		...
		sugestion=null;
		capacPocPostListas=capacitacionPocPostList.limpiar();
	}
	...
	public boolean tieneDocumento(Long idCapacitacion) {
	...
	@SuppressWarnings("unchecked")
	public String postularse(Long idCapacitacion){
		try {
			List<PostulacionCap> caps=em.createQuery("Select p from PostulacionCap p " +
					" where p.capacitacion.idCapacitacion is null and p.persona.idPersona="+usuarioLogueado.getPersona().getIdPersona()+" " +
					" and p.ficha=true and p.activo=true").getResultList();
			
			if(caps.isEmpty()){
				statusMessages.add(Severity.ERROR," Debe completar la Ficha de Inscripción en el enlace “Ficha de Inscripción” antes de postularse");
				return null;
			}
			
			return "/capacitacion/registrarImprimirPostulacion/RegistrarPostulacion.seam?capacitacionesIdCapacitacion="
			+ idCapacitacion;
	...
	public List<String> autocompletar(String suggest) {
		...
		List<CapacitacionPocPost> lista= new ArrayList<CapacitacionPocPost>();
		if(suggest!=null) 
			lista=em.createQuery("Select c from CapacitacionPocPost c where lower(c.nombre) like :desc ").setParameter("desc","%"+suggest+"%").getResultList();
		...
		for (CapacitacionPocPost o : lista) {
	...
	// GETTERS Y SETTERS
	public List<CapacitacionPocPost> getCapacPocPostListas() {
		return capacPocPostListas;
	}


	public void setCapacPocPostListas(List<CapacitacionPocPost> capacPocPostListas) {
		this.capacPocPostListas = capacPocPostListas;
	}
	...
	public String getFromCuenta() {
		return fromCuenta;
	}


	public void setFromCuenta(String fromCuenta) {
		this.fromCuenta = fromCuenta;
	}
	...
	public void setUsurioLogueado(Usuario usuarioLogueado) {
