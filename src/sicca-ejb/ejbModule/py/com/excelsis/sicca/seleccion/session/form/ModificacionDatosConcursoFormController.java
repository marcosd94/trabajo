...
@Name("modificacionDatosConcursoFormController")
public class ModificacionDatosConcursoFormController implements Serializable {
	...
	private void validarOee() {
		...
		String estados = seguridadUtilFormController.estadoActividades(
				"ESTADOS_GRUPO", "FIRMADO HOMOLOGACION") + "";
		estados += "#"
		...
		seguridadUtilFormController.verificarPerteneceOee(null,
				concursoPuestoAgr.getIdConcursoPuestoAgr(), estados,
	...
	public void init() {
		...
		fromConcurso = "seleccion/concurso/modificacionConcurso/ModificarDatosConcursoList.seam?concursoPuestoAgrIdConcursoPuestoAgr="
	...
	public String getQuery() {
		...
		ref1 = referenciasUtilFormController.getReferencia("ESTADOS_GRUPO",
				"FIRMADO HOMOLOGACION");
		Referencias ref2 = new Referencias();
		ref2 = referenciasUtilFormController.getReferencia("ESTADOS_GRUPO",
				"FINALIZADO CARGA");
		Referencias ref3 = new Referencias();
		ref3 = referenciasUtilFormController.getReferencia("ESTADOS_GRUPO",
				"CREADO EN ACTIVIDAD MODIFICAR");
		String query = "select concursoPuestoAgr from ConcursoPuestoAgr concursoPuestoAgr "
				...
				+ ref1.getValorNum()
				+ " or (concursoPuestoAgr.estado = "
				+ ref2.getValorNum()
				+ " and concursoPuestoAgr.homologar is false)"
				+ " or concursoPuestoAgr.estado = " + ref3.getValorNum() + ")";
		return query;
	}
	...
	private Boolean verificarPcd(Long id) {
		String sql = "select datos.* "
				+ "from seleccion.datos_grupo_puesto datos "
		...
		listaPcd = em.createNativeQuery(sql, DatosGrupoPuesto.class)
				.getResultList();
	...
	private Boolean cuentaConMatriz(Long id) {
		String sql = "SELECT M.* " + "FROM SELECCION.MATRIZ_DOC_CONFIG_ENC M "
				+ "WHERE M.ACTIVO IS TRUE " + "AND M.ID_CONCURSO_PUESTO_AGR = "
				+ id;
		List<MatrizDocConfigEnc> listDoc = new ArrayList<MatrizDocConfigEnc>();
		listDoc = em.createNativeQuery(sql, MatrizDocConfigEnc.class)
				.getResultList();
		if (listDoc.isEmpty())
			return false;
		return true;
	...
	public String nextTask() {
		...
		if (!estaConcursoEnFecha()) {
			...
			boolean presentacionConcurso = tienePresentacionConcurso(concurso
					.getIdConcurso());
			...
			for (ConcursoPuestoAgr sel : listaConcursoAgr) {
				if (sel.getEstado().intValue() != estadoCreado.intValue()) {
					if (!cumpleFechaConcurso(sel.getIdConcursoPuestoAgr())) {
						...
						String mensaje = "Debe cargar fechas al grupo: "
								+ sel.getDescripcionGrupo();
...
/*				if (!presentacionConcurso
						&& !tienePresentacionGrupo(sel.getIdConcursoPuestoAgr())) {
					statusMessages.clear();
					String mensaje = "Debe cargar Lugar de Presentación al grupo: "
							+ sel.getDescripcionGrupo();
					statusMessages.add(Severity.ERROR, mensaje);
				...
				if (sel.getEstado().intValue() == estadoCreado.intValue()) {
					...
					if (!cuentaConMatriz(sel.getIdConcursoPuestoAgr())) {
						...
						statusMessages
								.add(Severity.ERROR,
										"Debe configurar una matriz documental para el grupo");

						return null;
					}

					if (sel.getConcurso().getPcd() != null
							&& !sel.getConcurso().getPcd()) {
		...
		try {
			...
			/*control en vez de "tareaCerrada"*/
			ConcursoPuestoAgr aux2 = em.find(ConcursoPuestoAgr.class, this.idConcursoPuestoAgr);
			em.refresh(aux2);			
			if (aux2.getEstado().equals(ref.getValorNum())) return null;
			/*fin control*/
			...
			for (ConcursoPuestoAgr sel : listaConcursoAgr) {
				...
				if (conc.getFechaExpediente() == null
						&& conc.getNroExpediente() == null) {
				...
				// Se guardan las observaciones para cada grupo - INCIDENCIA
				// 1002
				if (observacion != null && !observacion.trim().isEmpty()) {
					...
					tablaObservacion.setUsuAlta(usuarioLogueado
							.getCodigoUsuario());
					tablaObservacion.setUsuMod(usuarioLogueado
							.getCodigoUsuario());
					tablaObservacion
							.setIdTaskInstance(jbpmUtilFormController
									.getIdTaskInstanceActual(sel
			...
			Referencias estadoCerrado = referenciasUtilFormController
					.getReferencia("ESTADOS_CONCURSO", "CERRADO GRUPOS");
			...
			for (ConcursoPuestoAgr sel : listaConcursoAgr) {
				...
				insertarPublicacionPortal(
						sel.getIdConcursoPuestoAgr(),
						concurso.getIdConcurso(), genTextoPublicacion());

				/****** Fin Incidencia 801 ********** */
				// Se pasa a la siguiente tarea
				jbpmUtilFormController.setConcursoPuestoAgr(sel);
				jbpmUtilFormController
						.setActividadActual(ActividadEnum.MODIFICAR_DATOS_CONCURSO);
				// jbpmUtilFormController.setActividadSiguiente(ActividadEnum.SOLICITAR_PUBLICACION);//MODIFICADO
				// RV
				jbpmUtilFormController
	...
	private Integer buscarMaxNroConcurso() {
		Date fechaActual = new Date();
		Integer anho = fechaActual.getYear() + 1900;
		String sql = "select max(c.numero) as nro "
				+ "from seleccion.concurso c " + "where c.anio = " + anho;
	...
	private boolean tienePresentacionConcurso(Long idConcurso) {
		String sql = "select * from seleccion.present_aclarac_doc "
				+ "where id_concurso = " + idConcurso;
		List<PresentAclaracDoc> lista = em.createNativeQuery(sql,
				PresentAclaracDoc.class).getResultList();
	...
	private boolean tienePresentacionGrupo(Long idGrupo) {
		String sql = "select * from seleccion.present_aclarac_doc "
				+ "where id_concurso_puesto_agr = " + idGrupo;
		List<PresentAclaracDoc> lista = em.createNativeQuery(sql,
				PresentAclaracDoc.class).getResultList();
		if (lista != null && lista.size() > 0)
			return true;
	...
	private Integer genNroExpediente() {
		...
		String query = "select max(concurso.nro_expediente)"
				+ " from seleccion.concurso concurso " + " where "
				+ "   concurso.anio = " + sdf.format(new Date())
	...
	public String genTextoPublicacion() {//MODIFICADO RV
		String texto;
		...
		String h1C = "</h1>";
		String br = "</br>";
		texto = hr + fechaPublicacion + h1O + "Período de Postulación:  "
	...
	public Boolean finalizarConcurso(Long idConcurso) {
		String query = "SELECT GRUPO.DESCRIPCION_GRUPO FROM SELECCION.CONCURSO_PUESTO_AGR GRUPO"
	...
	private Integer obtenerNroExpediente() {
		Date fechaActual = new Date();
		Integer anho = fechaActual.getYear() + 1900;
		String sql = "select max(nro_expediente) as nro "
				+ "from seleccion.concurso concurso "
				+ "where extract(year from concurso.fecha_expediente) = "
				+ anho;
	...
	private Boolean estaConcursoEnFecha() {
		String sql = "select fech_agr.*  "
				+ "from seleccion.fechas_grupo_puesto fech_agr  "
				+ "join seleccion.concurso conc  "
				+ "on conc.id_concurso = fech_agr.id_concurso "
				+ "where conc.id_concurso = " + concurso.getIdConcurso();
		List<FechasGrupoPuesto> listaFec = new ArrayList<FechasGrupoPuesto>();
		listaFec = em.createNativeQuery(sql, FechasGrupoPuesto.class)
				.getResultList();
	...
	private Boolean cumpleComisionConcurso(Long id) {
		String sql = "select com_grupo.* "
				+ "from seleccion.comision_grupo com_grupo "
				+ "join seleccion.concurso_puesto_agr agr "
		...
		try {
			...
			listaComision = em.createNativeQuery(sql, ComisionGrupo.class)
					.getResultList();
	...
	private Boolean cumpleDatosPuesto(Long id) {
		String sql = "select dato.* "
				+ "from seleccion.datos_grupo_puesto dato "
				+ "join seleccion.concurso_puesto_agr agr "
