...
@Name("datosHomologacionGrupoFC")
public class DatosHomologacionGrupoFC implements Serializable {
	...
	public void imprimirReporte() {
		try {
			...
			JasperReportUtils.respondPDF("Rpt_465CU",	obtenerMapaParametros(), false, conn, usuarioLogueado);
			conn.close();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private HashMap<String, Object> obtenerMapaParametros() {
		...
		if(idConcursoPuestoAgr!=null)
		{
			...
			String nroAnio= concurso.getNumero()==null?"-":concurso.getNumero()+"";
			nroAnio+="/";
			nroAnio+=concurso.getAnhio()==null?"-":concurso.getAnhio();
			param.put("concurso", nroAnio+" -"+concurso.getConfiguracionUoCab().getDescripcionCorta()+" - "+concurso.getNombre());
