...
@Name("firmarResolucionAdjudicacionFormController")
public class FirmarResolucionAdjudicacionFormController implements Serializable {
	...
	private List<ConcursoPuestoAgr> listaGruposAtrasados;
	...
	public Boolean buscarResolucion() {
		...
		String q = "select * from general.documentos where nombre_tabla like 'Resolucion' and nombre_pantalla like '"
				+ nombrePantalla + "' and id_concurso = " + idConcurso + " and activo = true ";
	...
	public void ListarConcursoPorEstado() {
		...
		for (int i = 0; i < listaGruposParaFirmar.size(); i++)
			seleccionados.add(i, listaGruposParaFirmar.get(i).getResolucionAdjudicacion()!=null);
	...
	// MODIFICADO RV
	public Boolean getSeleccionado(int index) {
		if(seleccionados.get(index) == listaGruposParaFirmar.get(index).getSeleccionado())
			return seleccionados.get(index);
		else
			return null;
	}
...
public boolean tareaCerrada(){
	...
	public String nextTask() {
		...
		for (int i = 0; i < concursoPuestoAgrs.size(); i++) {
			...
			if (grupo.getResolucionAdjudicacion() != null) {
				if (fromActividad.equals("7")) {
					nombrePantalla = "FirmaResolHomolog";
				...
				if (fromActividad.equals("21")) {
					...
					if (grupo.getResolucionAdjudicacion() == null)
						...
						idCU = grupo.getResolucionAdjudicacion()
								.getIdResolucion();
				}
				if (fromActividad.equals("26")) {
					nombrePantalla = "FirmaResolNombram";
					// idCU =
					// grupo.getResolucionNombramiento().getIdResolucion();
				}
				if (idCU != null) {
		 ...
		 * */
		/**
		 * if (fromActividad.equals("7")) { for (int i = 0; i <
		 * listaGruposParaFirmar.size(); i++) { ConcursoPuestoAgr agr =
		 * em.find(ConcursoPuestoAgr.class,
		 * listaGruposParaFirmar.get(i).getIdConcursoPuestoAgr());
		 * 
		 * 
		 * agr.setEstado(firmadoHomologacion());
		 * agr.setUsuMod(usuarioLogueado.getCodigoUsuario());
		 * agr.setFechaMod(new Date()); em.merge(agr);
		 * 
		 * 
		 * List<ConcursoPuestoDet> d = em .createQuery(
		 * "Select c from ConcursoPuestoDet c" +
		 * " where c.activo=true and  c.concursoPuestoAgr.idConcursoPuestoAgr="
		 * + listaGruposParaFirmar.get(i) .getIdConcursoPuestoAgr() +
		 * " and c.estadoDet.idEstadoDet=" + conDocumentoHomologacion())
		 * .getResultList(); for (int j = 0; j < d.size(); j++) {
		 * ConcursoPuestoDet puestoDet = em.find( ConcursoPuestoDet.class,
		 * d.get(j) .getIdConcursoPuestoDet());
		 ...
		 * em.merge(puestoDet);
		 * 
		 * if (d.get(j).getPlantaCargoDet() != null) { PlantaCargoDet pdet =
		 * em.find(PlantaCargoDet.class, d .get(j).getPlantaCargoDet()
		 * .getIdPlantaCargoDet());
		 * pdet.setEstadoDet(estadoFirmadoHomologacion()); pdet.setFechaMod(new
		 * Date()); pdet.setUsuMod(usuarioLogueado.getCodigoUsuario());
		 * em.merge(pdet);
		 * 
		 * } }
		 * 
		 * 
		 * List<HomologacionPerfilMatriz> matrizs = em .createQuery(
		 ...
		 * 
		 * for (int j = 0; j < matrizs.size(); j++) { HomologacionPerfilMatriz h
		 * = em.find( HomologacionPerfilMatriz.class, matrizs.get(j)
		 * .getIdHomologacion()); if (h.getFechaHomolog() != null) {
		 * calCalendario.setTimeInMillis(h.getFechaHomolog() .getTime()); // se
		 * suma un año a la fecha homologacion calCalendario.add(Calendar.YEAR,
		 * 1); h.setFechaLimite(calCalendario.getTime());
		 * h.setFechaVencimiento(calCalendario.getTime()); }
		 * h.setEstado("HOMOLOGADO");
		 * h.setUsuHomolog(usuarioLogueado.getCodigoUsuario());
		 * h.setFechaHomolog(new Date()); em.merge(h);
		 * 
		 * } Observacion observacion = new Observacion();
		 * observacion.setObservacion(obs); observacion.setFechaAlta(new
		 * Date()); observacion.setConcurso(em.find(Concurso.class,
		 * idConcurso));
		 ...
		 * em.persist(observacion); // em.flush();
		 * 
		 * }
		 * 
		 ...
		 * .get(0)); if (jbpmUtilFormController.nextTask()) { em.flush();
		 * 
		 * }
		 * 
		 * }
		...
		if (fromActividad.equals("21")) {
			...
			for (int i = 0; i < seleccionados.size(); i++) {
				if (seleccionados.get(i)) {
					concursoPuestoAgrs.add(p, listaGruposParaFirmar.get(i));
					p++;
				}
			}
	...
	public String mostrarEstado(int key) {
		...
		return this.referenciasUtilFormController.getReferenciaPorValorNum("ESTADOS_GRUPO", key).getDescLarga();
	}
	...
	public String anexar(Long idConcPuestoAgr) {
		...
		String isEdit = "true";
		String from = "seleccion/firmarResolucionHomologacion/FirmarResolucionAdjudicacion";
		...
		pagina += "&nombrePantalla=" + nombrePantalla + "&idConcurso="
				+ idConcurso + "&idConcursoPuestoAgr=" + idConcPuestoAgr;
	...
	private String serializarLista( List lista ) {
