...
@Name("elaborarPublicacionSeleccionadosFormController")
public class ElaborarPublicacionSeleccionadosFormController implements
		Serializable {
	 ...
	 */
	public void init() {
		if (evaluarDocPostulantesFormController == null) 
	...
	private void actualizarEstadosElegibles() {
		try {
			...
			jbpmUtilFormController
					.setActividadActual(ActividadEnum.PUBLICAR_ADJUDICADOS);
	...
	private Boolean existenElegibles() {
		...
		String sql = "SELECT c.* "
				+ " FROM seleccion.lista_elegibles_cab  c, "
				+ "seleccion.lista_elegibles_det   d  "
				+ "join seleccion.postulacion postulacion on postulacion.id_postulacion = d.id_postulacion "
				...
				+ "AND d.disponible is true and postulacion.activo is true and postulacion.usu_cancelacion is null ";
		try {
	...
	private void validarOee() {
		...
		seguridadUtilFormController.verificarPerteneceOee(
				...
				ActividadEnum.PUBLICAR_ADJUDICADOS.getValor());
	}
	...
	//HACE UN CONTROL PARA VER SI YA SE REALIZO ESTA ACTIVIDAD
	...
	public String nextTask() {
		...
		try {
			...
			jbpmUtilFormController
					.setActividadActual(ActividadEnum.PUBLICAR_ADJUDICADOS);
			//jbpmUtilFormController
					//.setActividadSiguiente(ActividadEnum.ELABORAR_RESOLUCION_ADJUDICACION);
			jbpmUtilFormController
					.setActividadSiguiente(ActividadEnum.FIRMAR_RESOLUCION_ADJUDICACION);
			jbpmUtilFormController.setTransition("next");
	...
	public void print() {
		...
		try {
			...
			else if(validarTipoConcurso())
			{
				...
				l.setParameter("tipo", "ADJUDICADO");
