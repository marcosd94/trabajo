...
@Name("administrarDecretoFormController")
public class AdministrarDecretoFormController implements Serializable {
	...
	private String nombrePantalla = "administrarDecreto_edit";
	...
	private void validarOee() {
		...
		seguridadUtilFormController.verificarPerteneceOee(null, concursoPuestoAgr.getIdConcursoPuestoAgr(), seguridadUtilFormController.estadoActividades("ESTADOS_GRUPO", "PERMANENTE N12")
			+ "", ActividadEnum.ELABORAR_DECRECTO_PRESIDENCIAL.getValor());
	}
	...
	private void ListarConcursoPorEstado() {
		...
		for (int i = 0; i < listaGruposParaFirmar.size(); i++)
			seleccionados.add(i, listaGruposParaFirmar.get(i).getDecreto()!=null);
	...
	public void changeSeleccionado(int index) {
		boolean valor  = !seleccionados.get(index);
		listaGruposParaFirmar.get(index).setSeleccionado(valor);
		seleccionados.set(index, valor);
	}
	...
	public String nextTask() {
		...
		puestoAgrList.clear();
		for( ConcursoPuestoAgr cpa : listaGruposParaFirmar )
			if(cpa.getDecreto() != null)
				puestoAgrList.add(cpa);
		for (int i = 0; i < puestoAgrList.size(); i++) {
			...
			jbpmUtilFormController.setActividadActual(ActividadEnum.ELABORAR_DECRECTO_PRESIDENCIAL);
			//jbpmUtilFormController.setActividadSiguiente(ActividadEnum.EDITAR_DOC_PRESIDENCIA_REPUBLICA);
	...
	private boolean tieneDecreto() {
		String q = "select * from general.documentos where documentos.activo is true and documentos.id_concurso="
				+ idConcurso + " and nombre_pantalla = 'administrarDecreto_edit'";
		return !em.createNativeQuery(q, Documentos.class)
...
/*		for (int i = 0; i < puestoAgrList.size(); i++) {
	...
	public String anexar(Long idConcPuestoAgr) {
			...
			String nombrePantalla = "administrarDecreto_edit";
			//String idCU = "";
			String nombreTabla = "Decreto";
			String mostrarDoc = "true";
			String isEdit = "true";
			String from = "seleccion/administrarDecreto/AdministrarDecreto";
