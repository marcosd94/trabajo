...
@Name("elaborarResolucionAdjudicacionFormController")
public class ElaborarResolucionAdjudicacionFormController implements Serializable {
	...
	private String nombrePantalla = "elaborarResolucionAdjudicacion_edit";
	...
	public void init() {
		...
		validarOee();
		//codActividad=ActividadEnum.ELABORAR_RESOLUCION_ADJUDICACION.getValor();
	...
	private void validaciones1y2(){
	 ...
	 public String endTask(ConcursoPuestoAgr cAgr){
		 try {
			...
			//jbpmUtilFormController.setActividadActual(ActividadEnum.ELABORAR_RESOLUCION_ADJUDICACION);
...
//		seguridadUtilFormController.verificarPerteneceOee(null, concursoPuestoAgr.getIdConcursoPuestoAgr(), seguridadUtilFormController.estadoActividades("ESTADOS_GRUPO", "ADJUDICADO")
//			+ "", ActividadEnum.ELABORAR_RESOLUCION_ADJUDICACION.getValor());
	}
	...
	public String nextTask() {
		...
		//jbpmUtilFormController.setActividadActual(ActividadEnum.ELABORAR_RESOLUCION_ADJUDICACION);
		jbpmUtilFormController.setActividadSiguiente(ActividadEnum.FIRMAR_RESOLUCION_ADJUDICACION);
	...
	private boolean validacion2(ConcursoPuestoAgr agr){
		...
		if(seleccionUtilFormController.cntAdjudicados(agr.getIdConcursoPuestoAgr())==0){
			...
			if(seleccionUtilFormController.cntElegible(agr.getIdConcursoPuestoAgr())==0){//no hay eleegible
				 ...
				 * */
				agr.setActivo(true); //para que aparezca en el historial de concursos
