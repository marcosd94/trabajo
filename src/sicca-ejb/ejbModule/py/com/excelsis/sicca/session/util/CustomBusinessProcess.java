...
@Name("customBusinessProcess")
@BypassInterceptors
@Install(dependencies="org.jboss.seam.bpm.jbpm", precedence=BUILT_IN)
public class CustomBusinessProcess extends AbstractMutable implements Serializable 
{
   ...
   public static CustomBusinessProcess instance()
   {
      if ( !Contexts.isConversationContextActive() )
      {
         throw new IllegalStateException("No active conversation context");
      }
      return (CustomBusinessProcess) Component.getInstance(CustomBusinessProcess.class, ScopeType.CONVERSATION);
   }
    ...
    * @param processDefinitionName the jBPM process definition name
    */
   public void createProcessAndTransition(String processDefinitionName, String transitionName)
   {
	   createProcessAndTransition(processDefinitionName, true, transitionName);
   }
    ...
    * @param processDefinitionName the jBPM process definition name
    */
   public void createProcessAndTransition(String processDefinitionName, boolean shouldSignalProcess, String transitionName)
   {
       ProcessInstance process = ManagedJbpmContext.instance().newProcessInstanceForUpdate(processDefinitionName);
       afterCreateProcess(processDefinitionName, process, shouldSignalProcess, transitionName);
   }
   ...
   private void afterCreateProcess(String processDefinitionName, ProcessInstance process, boolean shouldSignalProcess, String transitionName)
   {
      setProcessId( process.getId() );
      // need to set process variables before the signal
      Contexts.getBusinessProcessContext().flush();
