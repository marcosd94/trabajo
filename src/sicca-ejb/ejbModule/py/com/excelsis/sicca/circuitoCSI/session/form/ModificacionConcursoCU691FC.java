...
@Name("modificacionConcursoCU691FC")
public class ModificacionConcursoCU691FC {
	@In
	...
	private void asignarRolesTarea() {
		roles =
			seleccionUtilFormController.getRolesTarea(ActividadEnum.CSI_CARGA_GRUPO.getValor(), ProcesoEnum.CONCURSO.getValor());
	}
	...
	private void validarOee() {
		...
		seguridadUtilFormController.verificarPerteneceOeeCSI(null, concursoPuestoAgr.getIdConcursoPuestoAgr(), estados, ActividadEnum.CSI_MODIFICAR_DATOS_CONCURSO.getValor());
	...
	public void init() {
		...
		fromConcurso =
			"circuitoCSI/modificacionConcurso/ModificarDatosConcursoList.seam?concursoPuestoAgrIdConcursoPuestoAgr="
	...
	public String nextTask() {
		...
		try {
			...
			if (ref != null) {
				EstadoDet estadoDet =
					seleccionUtilFormController.buscarEstadoDet("CONCURSO", "MODIFICADO DATOS");
				for (ConcursoPuestoAgr sel : listaConcursoAgr) {
					buscarCargos(sel.getIdConcursoPuestoAgr());
					sel.setEstado(ref.getValorNum());
					sel.setFechaEvalDocHasta(new Date());
					em.merge(sel);

					// Actualizar el estado de los puestos
					if (sel.getConcursoPuestoDets() != null) {
						for (ConcursoPuestoDet concursoPuestoDet : sel.getConcursoPuestoDets()) {
		...
		//	}
			...
			jbpmUtilFormController.setActividadActual(ActividadEnum.CSI_MODIFICAR_DATOS_CONCURSO);
			jbpmUtilFormController.setActividadSiguiente(ActividadEnum.CSI_SOLICITAR_PUBLICACION);
	...
	public String genTextoPublicacion() {
		...
		texto.append(hr + fechaPublicacion + h1O + spanO + "Se informa que en fecha " + sdf.format(new Date())
			+ " el concurso se ha cerrado." + sdfFecha.format(new Date()) + "." + spanC + h1C);
