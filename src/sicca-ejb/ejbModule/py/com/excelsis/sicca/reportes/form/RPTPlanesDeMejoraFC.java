...
@Name("rptPlanesDeMejoraFC")
public class RPTPlanesDeMejoraFC implements Serializable{
...
//					+ nivelEntidadOeeUtil.getIdUnidadOrganizativa();
	...
	public void pdf() throws SQLException {
		...
		try {
			...
			JasperReportUtils.respondPDF("RPT_CU578_PLAN_MEJORA",
...
//			param.put("uo", nivelEntidadOeeUtil.getDenominacionUnidadOrgaDep());
	...
	private String getQueryReport() {
		SimpleDateFormat sdf = new SimpleDateFormat("dd-MM-yyyy");
		String query = "select rpt.* " +
				"from eval_desempeno.planes_mejora_rpt rpt "
				+ "where rpt.fecha_eval_desde >= to_date('"
				+ sdf.format(fechaDesde) + "','DD-MM-YYYY') ";
		query += " and rpt.fecha_eval_desde <= to_date('"
				+ sdf.format(fechaHasta) + "','DD-MM-YYYY') ";
		if(idEvaluacion != null)
			query += " and rpt.id_evaluacion_desempeno = "+idEvaluacion;
		if(idPlantilla != null)
			query += " and rpt.id_grupo_metodologia = "+idPlantilla;
		if(nivelEntidadOeeUtil.getCodNivelEntidad() != null)
			query += " and rpt.nen_cod = "+nivelEntidadOeeUtil.getCodNivelEntidad();
		
		if(nivelEntidadOeeUtil.getIdConfigCab() != null)
			query += " and rpt.id_configuracion_uo = "+nivelEntidadOeeUtil.getIdConfigCab();
//		if(nivelEntidadOeeUtil.getCodigoUnidOrgDep() != null)
//			query += " and rpt.cod_uo = '"+nivelEntidadOeeUtil.getCodigoUnidOrgDep()+"'";
		return query;
