...
@Name("rptSegunAntiguedadOee740FC")
public class RptSegunAntiguedadOee740FC {
	...
	private String obtenerSql() throws Exception {
		...
		String sql = "SELECT DISTINCT 	OEE.DENOMINACION_UNIDAD AS OEE, "+
					...
					"	SIN_ENTIDAD.ENT_NOMBRE AS ENT_NOMBRE,SNE.NEN_CODIGO||'.'||SIN_ENTIDAD.ENT_CODIGO||'.'||OEE.ORDEN AS ORDEN,SIN_ENTIDAD.ENT_CODIGO,OEE.ORDEN, "+
					...
					"	legajo.fnc_antiguedad_funcion_publica(e.id_persona) as antiguedad "+
		...
		if(desde!=null){
			if (!sufc.validarInput(desde.toString(), TiposDatos.INTEGER.getValor(), null)) {
				return null;
			}
			sql+=" and legajo.fnc_antiguedad_funcion_publica(e.id_persona)  >= "+desde;
		}
			
		if(hasta!=null){
			if (!sufc.validarInput(hasta.toString(), TiposDatos.INTEGER.getValor(), null)) {
				return null;
			}
			sql+=" and legajo.fnc_antiguedad_funcion_publica(e.id_persona)   <= "+hasta;
		}
			
		sql+=" order by SNE.NEN_CODIGO, SIN_ENTIDAD.ENT_CODIGO,OEE.ORDEN, "+
			"	UO.DENOMINACION_UNIDAD, P.APELLIDOS ";
		return sql;
	}
	...
	public void imprimir(String formato) throws Exception {
			if(desde==null){
				statusMessages.add(Severity.ERROR,"Debe ingresar el valor desde");
				return;
			}
			if(hasta==null){
				statusMessages.add(Severity.ERROR," Debe ingresar el valor hasta");
				return;
			}
			...
			try {
				...
				reportUtilFormController.setNombreReporte("RPT_CU740");
