...
@Name("rptPersonalSubsidioFamiliarCU741")
public class RptPersonalSubsidioFamiliarCU741 implements Serializable{
	/**
	 * 
	 */
	private static final long serialVersionUID = -7950799000076467464L;
	@In
	...
	public void imprimir(String formato) throws Exception {
		...
		try {
			...
			reportUtilFormController
					.setNombreReporte("RPT_CU741_Personal_Subsidio_Familiar");
	...
	private String getQuery() {
		String sql = "SELECT DISTINCT OEE.DENOMINACION_UNIDAD AS OEE, SNE.NEN_CODIGO AS NEN_COD, " +
				...
				"SIN_ENTIDAD.ENT_CODIGO,OEE.ORDEN, UO.DENOMINACION_UNIDAD UNIDAD_ORGANIZATIVA, SNE.NEN_CODIGO||'.'||SIN_ENTIDAD.ENT_CODIGO||'.'||OEE.ORDEN  || DESVINCULACION.GETDEPENDIENTES(UO.ID_CONFIGURACION_UO_DET) as codigo_uo, " +
				...
				"legajo.fnc_fecha_ingreso_funcion_publica(e.id_persona) as fecha_ingreso, p.fecha_nacimiento, SUBSTRING(AGE(NOW(),p.FECHA_NACIMIENTO)::TEXT FROM 1 FOR 2) EDAD,  " +
				"legajo.fnc_cantidad_hijos(p.id_persona ,"+ edadDesde+" , "+edadHasta+" ) as cantidad_hijos, p.id_persona, " +
				"substring(legajo.fnc_cantidad_hijos(p.id_persona ,"+ edadDesde+" , "+edadHasta+" ) from 1 for 2)::INTEGER hijos " +
				"FROM GENERAL.EMPLEADO_PUESTO E " +
				"JOIN GENERAL.PERSONA P ON E.ID_PERSONA = P.ID_PERSONA " +
				"join legajo.familiares f on f.id_persona = p.id_persona " +
				"join general.persona p_familiar on f.id_persona_familiar = p_familiar.id_persona " +
