...
FROM(	
		...
		FROM (	SELECT temp1.anho,temp1.mes,temp1.nivel,temp1.entidad,temp1.oee,
			SUM(case when (temp1.documento_identidad is not null) THEN 1 ELSE 0 END) AS documento_identidad,
		...
		FROM (
		SELECT DISTINCT tmp.documento_identidad,tmp.anho_ingreso,tmp.anho,tmp.mes,nivel,entidad,oee,obj_codigo
		FROM sinarh.sin_nivel_entidad sne 
		...
		AND tmp.mes = p_mes
		UNION ALL
		SELECT DISTINCT tmp1.documento_identidad,tmp1.anho_ingreso,tmp1.anho,tmp1.mes,nivel,entidad,oee,obj_codigo
		FROM sinarh.sin_nivel_entidad sne 
