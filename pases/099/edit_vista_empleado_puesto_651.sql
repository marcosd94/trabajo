...
CREATE OR REPLACE VIEW general.empleado_puesto_651 AS 
         SELECT configuracion_uo_cab.denominacion_unidad AS oee, configuracion_uo_det.denominacion_unidad AS uo, planta_cargo_det.descripcion AS puesto,
         empleado_puesto.fecha_inicio, empleado_puesto.fecha_fin, datos_especificos.descripcion AS estado, empleado_puesto.id_persona, NULL::unknown AS id,
...
UNION 
         ...
         empleado_puesto.fecha_inicio, empleado_puesto.fecha_fin, CASE WHEN empleado_puesto.contratado = TRUE THEN 'CONTRATADO' ELSE 'PERMANENTE' END AS estado, empleado_puesto.id_persona, empleado_puesto.id_empleado_puesto AS id,
         NULL::unknown AS tipo
           FROM general.empleado_puesto
